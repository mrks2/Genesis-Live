# 🛡️ GENESIS LIVE — Modération de contenu

*Protéger la communauté, gérer les abus, maintenir un environnement sain 24/7.*

---

## 📖 Table des matières

1. [Philosophie de modération](#philosophie-de-modération)
2. [Types de contenu à modérer](#types-de-contenu-à-modérer)
3. [Niveaux de sanctions](#niveaux-de-sanctions)
4. [Modération automatique](#modération-automatique)
5. [Modération humaine](#modération-humaine)
6. [Noms d'entités créées par les viewers](#noms-dentités-créées-par-les-viewers)
7. [Modération des raids et brigades](#modération-des-raids-et-brigades)
8. [Gestion des minors](#gestion-des-minors)
9. [Cas spéciaux et edge cases](#cas-spéciaux-et-edge-cases)
10. [Transparence et appel](#transparence-et-appel)
11. [Équipe de modération](#équipe-de-modération)
12. [Outils et dashboards](#outils-et-dashboards)
13. [Protocoles de crise](#protocoles-de-crise)
14. [Conformité légale](#conformité-légale)
...
Genesis Live est un **espace créatif partagé**. Les viewers co-créent l'histoire du monde. Cette création collective **ne peut pas se faire** si certains viewers ont peur de participer, sont harcelés, ou voient leur créativité salie par du contenu inapproprié.

La modération n'est pas une **restriction de liberté**, c'est une **condition de la liberté créative de tous**.

### Les 5 principes de modération

**1. Clarté avant tout**
Les règles sont **simples, publiques, et expliquées**. Aucune zone grise que les viewers devraient deviner.

**2. Graduation des sanctions**
On ne bannit pas pour un premier faux pas. Sauf pour les violations graves, on avertit, on explique, on donne une chance.

**3. Transparence des décisions**
Quand on bannit, on **explique pourquoi** (publiquement ou en DM selon le cas). Pas de "disparitions" inexpliquées.

**4. Protection des victimes avant les agresseurs**
Entre un viewer qui harcèle et un viewer harcelé, la priorité est **toujours** à la victime. Pas de "deux côtés de l'histoire" quand il y a une victime claire.

**5. Cohérence dans l'application**
Les mêmes règles pour tous. Pas de favoritisme pour les subs, donateurs, ou amis.

### Ce que la modération N'EST PAS

❌ **De la censure politique** : les opinions politiques divergentes sont OK tant qu'elles ne violent pas les règles
❌ **Un jugement moral global** : on modère des **comportements**, pas des **personnes**
❌ **Imposer un consensus** : les désaccords, les débats sont bienvenus
❌ **Punir la maladresse** : on distingue bonne foi et mauvaise intention
❌ **Traiter les viewers comme des enfants** : on respecte leur intelligence
...
### 🔴 Catégorie 1 — Violations absolues (ban immédiat)
...
### 🟠 Catégorie 2 — Violations graves (timeout long + warning)
...
### 🟡 Catégorie 3 — Violations modérées (timeout court ou warning)
...
### 🟢 Catégorie 4 — Comportements limites (rappel gentil)
...
| Niveau | Sanction | Durée | Cas d'usage |
|--------|----------|-------|-------------|
| 0 | Warning public | Instantané | Catégorie 4, premier avertissement |
| 1 | Warning privé (DM) | Instantané | Catégorie 3, contexte flou |
| 2 | Timeout court | 10 min | Catégorie 3, récidive du niveau 0-1 |
| 3 | Timeout moyen | 1 heure | Récidive, ou Catégorie 3 sérieuse |
| 4 | Timeout long | 24 heures | Catégorie 2 modérée |
| 5 | Ban temporaire | 7 jours | Catégorie 2 grave, récidive |
| 6 | Ban temporaire étendu | 30 jours | Récidive multiple |
| 7 | Ban permanent | À vie | Catégorie 1, récidive extrême |
...
### Cooldown des violations

Les violations ne restent pas en mémoire indéfiniment :

- **Catégorie 4** : efface après 30 jours
- **Catégorie 3** : efface après 90 jours
- **Catégorie 2** : efface après 1 an
- **Catégorie 1** : **jamais effacé** (si ban permanent n'a pas été appliqué, c'est une exception rare)
...
Le système de modération automatique est **en première ligne**. Il traite des milliers de messages/jour dont 99% sont légitimes.
...
Message reçu
     ...
     ▼
┌─────────────────────────┐
│ 1. Filtres techniques   │ → Trop long ? Vide ? Blacklist ?
└────────┬────────────────┘
         │ Pass
         ▼
┌─────────────────────────┐
│ 2. Détection patterns   │ → Spam ? Caps ? Répétition ?
└────────┬────────────────┘
         │ Pass
         ▼
┌─────────────────────────┐
│ 3. Analyse sémantique   │ → Mots-clés suspects, contexte
└────────┬────────────────┘
         │ Pass
         ▼
┌─────────────────────────┐
│ 4. Évaluation globale   │ → Historique viewer, confiance
└────────┬────────────────┘
         │
         ▼
   Action appropriée
...
### Filtres techniques (niveau 1)

```typescript
function technicalFilter(msg: Message): FilterResult {
  // Trop long
  if (msg.text.length > 500) {
    return { action: 'block', reason: 'too_long' };
  }
  ...
  // Blacklist technique (URLs non autorisées, caractères interdits)
  if (containsURL(msg.text) && !viewer.isModerator && !viewer.isVIP) {
    return { action: 'block', reason: 'url_not_allowed' };
  }
  
  // Caractères Unicode suspects (zalgo text, invisibles)
  if (hasZalgoText(msg.text)) {
    return { action: 'block', reason: 'zalgo_text' };
  }
...
function detectSpam(viewer: Viewer, msg: Message): boolean {
  ...
  // Trop de messages identiques
  const duplicates = recent.filter(m => m.text === msg.text).length;
  if (duplicates >= 3) return true;
  
  // Trop de messages en général
  if (recent.length > 10) return true;
  
  // Messages quasi-identiques (similarité > 90%)
  const similar = recent.filter(m => similarity(m.text, msg.text) > 0.9).length;
...
function checkBlockedWords(text: string): string | null {
  ...
  for (const word of BLOCKED_WORDS.absolute) {
...
const BLOCKED_WORDS = {
  ...
  absolute: [
    // Insultes raciales, homophobes, etc.
    // Menaces de mort
    // Contenu pédocriminel
    // [Liste sensible à maintenir]
  ],
  
  // Niveau 2 : bloqués sauf contexte explicitement autorisé
  contextual: [
    // Mots ambigus qui peuvent être OK dans certains contextes
  ],
  
  // Niveau 3 : warning (passent mais logués pour analyse)
  warning: [
    // Mots à surveiller pour patterns
  ]
};
```

**Détection de haine identitaire** avec ML (optionnel, avancé) :
Utiliser une API de modération comme OpenAI Moderation API, ou un modèle open source fine-tuné. Retourne un score pour différentes catégories.

```typescript
async function analyzeWithAI(text: string): Promise<ModerationScores> {
  // Score 0-1 pour chaque catégorie
  return {
    hate: 0.02,
    harassment: 0.15,
    violence: 0.01,
...
### Évaluation globale (niveau 4)

Combine tous les signaux avec l'historique du viewer :

```typescript
interface ViewerTrust {
  accountAge: number;           // Jours depuis création
  totalMessages: number;        // Activité historique
  violationCount: number;       // Infractions passées
  goodBehaviorStreak: number;   // Jours sans infraction
  isSubscriber: boolean;
  isVIP: boolean;
  isBroadcaster: boolean;
  trustScore: number;           // 0-100 calculé
}

function calculateTrust(viewer: Viewer): number {
  let score = 50; // baseline
  
  // Ancienneté (+20 max)
  score += Math.min(viewer.accountAge / 365 * 20, 20);
  
  // Activité historique (+15 max)
  score += Math.min(Math.log10(viewer.totalMessages + 1) * 5, 15);
  
  // Historique propre (+20 max)
  if (viewer.violationCount === 0) score += 10;
  if (viewer.goodBehaviorStreak > 30) score += 10;
  
  // Infractions (-50 max)
  score -= viewer.violationCount * 10;
  
  // Statut (+5)
  if (viewer.isSubscriber) score += 5;
  if (viewer.isVIP) score += 10;
...
- Trust < 20 : surveillance accrue, sanctions immédiates
...
4. Considérer la **catégorie** de la violation
...
- Laisser des biais personnels influencer (fatigue, stress)
...
- Reconnaissance publique du travail fait

---

## Noms d'entités créées par les viewers

### Le problème spécifique

Les viewers peuvent nommer des entités (cités, espèces, religions). Ces noms **deviennent partie du monde permanent** de Genesis Live. Un nom problématique persiste **pour toujours** dans les archives.

### Validation au moment de la création

```typescript
async function validateEntityName(name: string, viewer: Viewer): Promise<ValidationResult> {
  // Longueur
  if (name.length < 2 || name.length > 30) {
    return { valid: false, reason: 'length', message: 'Le nom doit faire entre 2 et 30 caractères.' };
  }
  
  // Caractères autorisés
  if (!/^[a-zA-Z0-9_\- àâçéèêëîïôûùñ]+$/.test(name)) {
    return { valid: false, reason: 'invalid_chars', message: 'Caractères non autorisés.' };
  }
  
  // Blacklist
  const blockedMatch = checkBlockedWords(name);
  if (blockedMatch) {
    return { valid: false, reason: 'blocked_word', message: 'Ce nom n\'est pas autorisé.' };
  }
  
  // Patterns problématiques
  if (isPersonalNameOfFamous(name)) {
    // Noms de personnes réelles (politiciens, célébrités)
    return { valid: false, reason: 'real_person', message: 'Pas de noms de personnes réelles.' };
  }
  
  if (containsSlur(name)) {
    return { valid: false, reason: 'slur', message: 'Ce nom contient une insulte.' };
  }
  
  // Nom déjà utilisé (éviter confusion et squatting)
  if (await nameAlreadyExists(name)) {
    return { valid: false, reason: 'already_used', message: 'Ce nom est déjà pris.' };
  }
  
  // Trust score insuffisant pour nommer ?
  if (viewer.trustScore < 30) {
    return { valid: false, reason: 'low_trust', message: 'Augmente ta présence avant de pouvoir nommer des entités.' };
  }
...
Si un nom est signalé 3+ fois, il est **mis en revue manuelle** (vue par modérateur).
...
- Si c'était malveillant → sanction sur le viewer
...
- `[mot raciste]` (insulte)
- `Tom_est_nul` (attaque autre viewer)
- `JesusChrist` (nom religieux réel)
...
## Modération des raids et brigades
...
- Chrono suspect (tout le monde arrive en 30 secondes)

### Détection automatique

```typescript
class RaidDetector {
  private newViewersWindow: Viewer[] = [];
  
  onViewerJoin(viewer: Viewer) {
    this.newViewersWindow.push(viewer);
    this.cleanupOldEntries();
    
    // Alerte si pic anormal
    const recentJoins = this.newViewersWindow.filter(v => 
      Date.now() - v.joinedAt < 60_000 // 1 min
    );
    
    if (recentJoins.length > 50) {
      // Plus de 50 nouveaux viewers en 1 min
      this.alertModerators({
        type: 'RAID_SUSPECTED',
        severity: 'high',
        details: `${recentJoins.length} new viewers in 1 minute`
      });
      
      // Activer automatiquement mode "slow mode"
...
### Protocole de raid hostile

**Détection** → **Mode défensif** → **Analyse** → **Décision**

1. **Détection** : alerte automatique ou par modo
2. **Mode défensif activé immédiatement** :
   - Slow mode 30s
   - Follower-only activé
   - Mods alertés
3. **Analyse** (5-15 minutes) :
   - Identifier les leaders du raid
   - Collecter les preuves
   - Déterminer la source (autre stream ? communauté externe ?)
4. **Décision** :
   - Bans permanents des instigateurs
   - Timeouts des suiveurs
   - Retour progressif à la normale (slow mode désactivé quand calme)
5. **Post-mortem** :
   - Documenter ce qui s'est passé
   - Identifier les améliorations système
...
Un stream public peut attirer des viewers mineurs **sans qu'on le sache**. Cela impose des responsabilités particulières.
...
- Thèmes adultes abordés avec nuance

**Classification cible** : équivalent PEGI 12 / ESRB T (Teen)
...
2. **Activer mode protection** sur ce compte :
   ...
   - Messages filtrés plus strictement
3. Si mineur clairement inapproprié (< 13 ans) :
   - Suggérer gentiment qu'il y a des streams plus adaptés à son âge
   - Ne pas engager conversation inappropriée
4. **Jamais de messages privés** à un mineur
...
4. Option : séparer (timeouts simultanés pour apaiser)

**Règle d'or** : jamais "c'est des deux côtés" si l'un harcèle clairement l'autre.
...
4. **Pas de show** : ne pas mettre cette personne en avant publiquement contre son gré
...
Tu viens de recevoir un warning pour [raison spécifique].

[Règle concernée]

Pas de sanction pour l'instant, mais si ça se reproduit, un timeout sera appliqué.

Si tu penses que c'est une erreur, tu peux en parler dans [canal d'appel].

Bonne suite sur Genesis Live !
```
...
Tu as été banni [durée / définitivement] pour [raison spécifique].

[Règle concernée et explication]

[Si applicable : conditions de retour]

Si tu penses que c'est une erreur ou que tu veux faire appel, contacte [canal d'appel].
```

### Système d'appel

Chaque sanction peut être **contestée** :
...
- Sanction alourdie (rare, si appel révèle aggravation)
...
- Taux d'appel
- Taux de réversion d'appel

Montre que la modération est **sérieuse et équitable**.
...
- Maturité dans les interactions
- Disponibilité sur les créneaux de stream
...
- Shadow session : observe un mod pendant 2-3 streams
...
- Actions rapides (timeout, ban, delete)
...
- Screenshots automatiques des moments litigieux
...
- Doxxing en cours
- Incident technique + exploitation par trolls
...
4. Ping du streamer et autres mods
...
4. **SIGNALER** :
   - À la plateforme (YouTube/Twitch)
   - À PHAROS (France) ou équivalent
   - À INTERPOL si international
5. **Informer** le streamer
6. **Ne pas mentionner publiquement** ce qui s'est passé (sauf "incident géré")
...
2. **Supprimer** tout le contenu hostile
3. **Bans** rapides des attaquants
4. **Coordination** avec la plateforme si graves
5. **Support** continu à la victime

### Scénario 4 — Bug exploitable
...
4. **Sanctions** selon la gravité (des warnings aux bans)
5. **Patch** du bug
6. **Communication** transparente : "Un bug a été exploité, voici ce qu'on fait"
...
4. Documenter toute la procédure
...
La modération n'est pas un rôle **populaire**, mais elle est **essentielle**. Sans elle, Genesis Live devient toxique en quelques semaines.

### Principes à ne jamais violer

1. **Cohérence** : les mêmes règles pour tous
2. **Transparence** : expliquer les décisions
3. **Protection des victimes** avant tout
4. **Documentation** de toutes les actions
5. **Signalement légal** obligatoire quand la loi l'impose

### Principes à adapter selon l'évolution

1. **Nouvelles règles** en réponse aux nouveaux problèmes
2. **Calibration des sanctions** selon ce qui fonctionne
3. **Taille de l'équipe** selon l'audience
4. **Outils** qui s'améliorent
...
- [ ] Dashboard modération opérationnel
- [ ] Au moins 2 mods recrutés et formés
- [ ] Discord privé mods créé
...
- [ ] Statistiques de modération configurées
- [ ] Relations avec autres streamers pour coordination

### Le mot de la fin

Modérer, c'est **tracer les limites du possible** pour que la liberté créative puisse exister. C'est un travail ingrat, souvent invisible, mais **fondateur**.

Une bonne modération se remarque par son **absence** : si tout le monde s'entend bien et que le chat est agréable, c'est que les mods font bien leur job.

Traite ton équipe de mods avec le **respect et la gratitude** qu'elle mérite. Sans elle, Genesis Live ne dure pas un mois.

---

*Document de modération — v1.0*
*À relire régulièrement. À adapter selon les évolutions. Non négociable sur ses principes fondamentaux.*
